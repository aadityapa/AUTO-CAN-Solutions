...
Requires **Node.js 18+** (CI and Vercel build on Node 20).
...
**CI** — `.github/workflows/ci.yml` runs a clean install, lint, and build on every push and
pull request, then checks that all nine routes prerendered, the sitemap is well-formed and
points at the production domain, and no placeholder text reached the HTML.
...
4. `FORMSUBMIT_TOKEN` → the per-address token from your FormSubmit dashboard, which keeps the email address out of the JavaScript bundle.
